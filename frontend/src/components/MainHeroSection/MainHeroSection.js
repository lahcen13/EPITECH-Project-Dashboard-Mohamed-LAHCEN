import React from 'react';
import PropTypes from 'prop-types';
import styles from './MainHeroSection.module.css';

const MainHeroSection = (props) => (
  <div className={styles.MainHeroSection}>
     <div class="px-4 py-5 my-5 text-center">
    <h1 class="display-5 fw-bold">{props.heading}</h1>
    <div class="col-lg-6 mx-auto">
      <p class="lead mb-4">{props.description}</p>
      <div class="d-grid gap-2 d-sm-flex justify-content-sm-center">
        <button type="button" class="btn btn-primary btn-lg px-4 gap-3">{props.ctaone}</button>
        <button type="button" class="btn btn-outline-secondary btn-lg px-4">{props.ctatwo}</button>
      </div>
    </div>
  </div>
  </div>
);

MainHeroSection.propTypes = {};

MainHeroSection.defaultProps = {};

export default MainHeroSection;
